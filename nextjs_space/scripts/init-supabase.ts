...
// Create Supabase admin client for direct SQL execution
...
// Default pipeline stages to insert
...
async function checkTablesExist(): Promise<boolean> {
  try {
    ...
    if (error?.message?.includes('relation') || error?.message?.includes('does not exist')) {
      console.log('ℹ️  Tables do not exist, will create them')
      return false
    }
    ...
    console.log('⚠️  Error checking tables:', error)
    return false
  }
}
...
async function initializeSupabase() {
  try {
    ...
    if (tablesExist) {
      ...
      // Still ensure default pipeline stages exist
      ...
      if (!stages || stages.length === 0) {
        ...
        console.log('✅ Pipeline stages inserted')
      }
    ...
    if (stagesError && !stagesError.message?.includes('already exists')) {
      console.error('⚠️  Warning inserting pipeline stages:', stagesError)
    } else {
      console.log(`✅ ${insertedStages?.length || DEFAULT_PIPELINE_STAGES.length} pipeline stages created`)
    }
    
    console.log('✅ Supabase initialization completed successfully!')
    console.log('🎉 Your CRM is ready to use!')
